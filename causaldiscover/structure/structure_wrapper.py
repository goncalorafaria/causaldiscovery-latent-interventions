...
from causaldiscover.structure.permutation import matching, PLDistribution
from causaldiscover.utils.flow_utils import squareplus
...
class PermutationMethod(CausalStructure):
    def __init__(self,
        scm,
        graph_prior,
        estimator="r",
        hard=False):
        super(PermutationMethod, self).__init__(
            scm=scm,
            graph_prior=graph_prior,
            estimator = estimator)

        # self.logpermutation_scale = torch.nn.Parameter(
        #        torch.randn(self.n)
        #    )
        ...
        self.sortdist = PLDistribution(n=self.n,hard=hard)

    def permute(self, x, temperature):

        P, emp_kl = self.sortdist.rsample(
            batch=x.shape[0],
            temperature=temperature)

        #empirical_kl = pl_dist.log_prob(P) - pl_dist_prior.log_prob(P)

        x_ord = torch.einsum("bi, bij -> bj",x,P)

        #     empirical_kl = pl_dist.log_prob(good_pm) - pl_dist_prior.log_prob(good_pm)
        return x_ord, P, emp_kl
...
class ECNOMethod(CausalStructure):
    ...
    def __init__(self, 
        ...
        super(ECNOMethod, self).__init__(
                scm=scm,
                graph_prior=graph_prior,
                estimator = estimator)

        edge_mask = torch.ones((self.n,self.n))-torch.eye(self.n)
        mask = torch.ones(self.n, self.n)
        mask = torch.tril(mask, diagonal=-1).bool()
        ...
        full_adjacency = torch.ones((scm.n,scm.n)) - torch.eye(scm.n)
        constraint_normalization = self.compute_constraint(full_adjacency)
    ...
    def get_probs(self):
        ...
        masked_gamma = self.gamma.clone()
        masked_gamma[~self.orientation_mask] = 0.0
        masked_gamma = masked_gamma + masked_gamma.T
        masked_gamma[~self.edge_mask] = float('-inf')
        p_edge = masked_gamma.sigmoid()

        masked_theta = self.theta.clone()
        masked_theta[~self.orientation_mask] = 0.0
        masked_theta = masked_theta - masked_theta.T
        masked_theta[~self.edge_mask] = float('-inf')
        p_orientation = masked_theta.sigmoid()

        return clamp_probs(p_orientation * p_edge)          
        
    def get_graph(self, batch, temperature):
        z, _ = super(ECNOMethod,self).get_graph(batch, temperature)
        ...
        l = (0.005 - torch.minimum(self.h_cache,temperature*0.005))/0.005
    ...
    def regularizer(self, z, mixer_percentage):
        ...
        h = self.n**2 / 2 *self.compute_constraint(probs)/ self.constraint_normalization
    ...
    def compute_constraint(self, probs):
        ...
        h = ( torch.diagonal(
            torch.matrix_exp(probs), dim1=-2, dim2=-1).sum(-1) - self.ntensor).mean()
...
fpn = torch.nn.Linear(
    in_features=self.d*2,
    out_features=self.d)

self.graph_forward = lambda perturbed_gamma : torch.nn.sigmoid(
    fpn( 
        recurrent(input=perturbed_gamma) 
        )
    )
"""
